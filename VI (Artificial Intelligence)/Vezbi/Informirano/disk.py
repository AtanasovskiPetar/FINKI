...
class DiskProblem(Problem):
    ...
    def goal_test(self, state):
        curr_list = list(state)
        initial_list = list(self.initial)
        initial_list.reverse()
        if curr_list == initial_list:
            return True
        else:
            return False

    def h(self, node):
        curr_list = list(node.state)
        initial_list = list(self.initial)
        initial_list.reverse()
        counter = 0
        for i in range(len(curr_list)):
            if str(curr_list[i]) != str(initial_list[i]):
                counter += 1
        return counter
...
if __name__ == "__main__":
    ...
    for i in range(num_el):
        start_list.append('Disk ' + str(i + 1))

    for i in range(length - num_el):
        start_list.append('0')

    start_state = tuple(start_list)

    disk_problem = DiskProblem(start_state)
    print(astar_search(disk_problem).solution())

# ['D1: Disk 3', 'D1: Disk 3', 'D2: Disk 1', 'D1: Disk 1', 'D2: Disk 1', 'D1: Disk 1', 'D1: Disk 2', 'D1: Disk 2', 'D2: Disk 2']
