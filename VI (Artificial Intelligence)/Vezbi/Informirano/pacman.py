...
class Pacman(Problem):
    ...
    @staticmethod
    def get_state(direction, new_position, dots):
        new_dots = list(dots)
        if new_position in dots:
            new_dots.remove(new_position)
        new_state = (direction, new_position, tuple(new_dots))
        return new_state

    def check_valid(self, state):
        pos = state[1]
        x, y = pos
        if 0 <= x < 10 and 0 <= y < 10 and pos not in self.obstacles:
            return True
        else:
            return False

    def successor(self, state):
        direction, position, dots = state
        x, y = position
        successors = dict()
        ...
        if direction == 'down':
            new_position = (x, y - 1)
            new_state = self.get_state(direction, new_position, dots)
            if self.check_valid(new_state):
                successors['ProdolzhiPravo'] = new_state

            new_position = (x, y + 1)
            new_state = self.get_state('up', new_position, dots)
            if self.check_valid(new_state):
                successors['ProdolzhiNazad'] = new_state

            new_position = (x + 1, y)
            new_state = self.get_state('right', new_position, dots)
            if self.check_valid(new_state):
                successors['SvrtiLevo'] = new_state

            new_position = (x - 1, y)
            new_state = self.get_state('left', new_position, dots)
            if self.check_valid(new_state):
        ...
        # direction == up
            ...
            new_position = (x, y - 1)
            new_state = self.get_state('down', new_position, dots)
            if self.check_valid(new_state):
                successors['ProdolzhiNazad'] = new_state
    ...
    def h(self, node):
        ...
        y = node.state[1][1]
        distances_list = list()
        for obstacle in node.state[2]:
            distances_list.append(self.manhattan_distance(x, obstacle[0], y, obstacle[1]))
...
if __name__ == "__main__":
    ...
    start = (direct, start_pos, tuple(dot))
    obst = (
        (0, 6), (0, 8), (0, 9), (1, 2), (1, 3), (1, 4), (1, 9), (2, 9), (3, 6), (3, 9), (4, 1), (4, 5), (4, 6), (4, 7),
        (5, 1), (5, 6), (6, 0), (6, 1), (6, 2), (6, 9), (8, 1), (8, 4), (8, 7), (8, 8), (9, 4), (9, 7), (9, 8))
    pacman_problem = Pacman(start, obst)
    print(astar_search(pacman_problem).solution())