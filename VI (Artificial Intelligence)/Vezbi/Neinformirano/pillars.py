...
class Pillars(Problem):
    ...
    def successor(self, state):
        ...
        for i in range(len(pillars)):
            pillar = pillars[i]
            if len(pillar) > 0:
                last_element = pillar[-1]
                for j in range(len(pillars)):
                    other_pillar = pillars[j]
                    if len(other_pillar) == 0 or last_element < other_pillar[-1]:
                        new_curr_pillar = pillar[:len(pillar) - 1]
                        list_other = list(other_pillar)
                        list_other.append(last_element)
                        new_other_pillar = tuple(list_other)
                        new_state = list()
                        for k in range(len(pillars)):
                            if k == i:
                                new_state.append(new_curr_pillar)
                            elif k == j:
                                new_state.append(new_other_pillar)
                            else:
                                new_state.append(pillars[k])
                            k += 1
                        new_state = tuple(new_state)
                        if new_state not in self.passed_states:
                            successors['MOVE TOP BLOCK FROM PILLAR ' + str(i+1) + ' TO PILLAR ' + str(j+1)] = tuple(new_state)
...
if __name__ == "__main__":
    start_pillars = input().split(";")
    end_pillars = input().split(";")
    start_state = list()
    end_state = list()

    for pill in start_pillars:
        if pill != '':
            list_int_pill = list()
            for block in pill.split(','):
                list_int_pill.append(int(block))
            start_state.append(tuple(list_int_pill))
        else:
            start_state.append(tuple(pill))

    for pill in end_pillars:
        if pill != '':
            list_int_pill = list()
            for block in pill.split(','):
                list_int_pill.append(int(block))
            end_state.append(tuple(list_int_pill))
        else:
            end_state.append(tuple(pill))

    pillars_problem = Pillars(tuple(start_state), tuple(end_state))
    result = breadth_first_tree_search(pillars_problem)
    if result:
        print('Number of action '+str(result.depth))
