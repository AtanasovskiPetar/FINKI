...
class Coloring(Problem):
    ...
    def successor(self, state):
        ...
        for index in range(len(state)):
            ...
            i, j, element = new_state[index]
            # curr
            new_changed_element = list(new_state[index])
            if element == 0:
                new_changed_element[2] = 1
            else:
                new_changed_element[2] = 0
            new_state[index] = tuple(new_changed_element)

            # left
            if j != 0:
                new_changed_element = list(new_state[index - 1])
                if new_changed_element[2] == 0:
                    new_changed_element[2] = 1
                else:
                    new_changed_element[2] = 0
                new_state[index - 1] = tuple(new_changed_element)
            ...
            # up
            if i != 0:
                new_changed_element = list(new_state[index - self.board_size])
                if new_changed_element[2] == 0:
                    new_changed_element[2] = 1
                else:
                    new_changed_element[2] = 0
                new_state[index - self.board_size] = tuple(new_changed_element)
            ...
            successors['x: '+str(i)+', y: '+str(j)] = tuple(new_state)
...
if __name__ == "__main__":
    ...
    for i in range(N):
        for j in range(N):
            start_state.append((i, j, int(start[index])))
            index += 1

    start_state = tuple(start_state)
    coloring_problem = Coloring(start_state, N)
    print(breadth_first_graph_search(coloring_problem).solution())
