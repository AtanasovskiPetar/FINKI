...
class Squares(Problem):
    ...
    def successor(self, state):
        ...
        for i in range(len(state)):
            name = 'kvadratche ' + str(i + 1)
            square = state[i]

            # up
            new_square_pos = (square[0], square[1] + 1)
            new_state = list(state)
            new_state[i] = new_square_pos
            new_state = tuple(new_state)
            if self.check_valid(new_state):
                succ['Pomesti ' + name + ' gore'] = new_state
            ...
            new_state = tuple(new_state)
            if self.check_valid(new_state):
                succ['Pomesti ' + name + ' dolu'] = new_state

            # left
            new_square_pos = (square[0] - 1, square[1])
            new_state = list(state)
            new_state[i] = new_square_pos
            new_state = tuple(new_state)
            if self.check_valid(new_state):
                succ['Pomesti ' + name + ' levo'] = new_state
            ...
            new_state = tuple(new_state)
            if self.check_valid(new_state):
                succ['Pomesti ' + name + ' desno'] = new_state
