...
class Snake(Problem):
    ...
    @staticmethod
    def get_state(direction, new_head, green_apples, positions):
        ...
        if new_head in green_apples:
            ...
            new_positions.remove(new_positions[-1])
        new_positions.insert(0, new_head)
        new_state = (direction, tuple(new_positions), tuple(new_green))
        return new_state
    ...
    def check_valid(self, state):
        ...
        head = positions[0]
        x, y = head
        if 0 <= x < 10 and 0 <= y < 10 and head not in self.red_apples and head not in positions[1:]:
            return True
        else:
            return False

    def successor(self, state):
        direction, positions, green_apples = state
        x, y = positions[0]
        successors = dict()
        ...
        if direction == 'down':
            new_head = (x, y - 1)
            new_state = self.get_state(direction, new_head, green_apples, positions)
            if self.check_valid(new_state):
                successors['ProdolzhiPravo'] = new_state

            new_head = (x - 1, y)
            new_state = self.get_state('left', new_head, green_apples, positions)
            if self.check_valid(new_state):
                successors['SvrtiDesno'] = new_state

            new_head = (x + 1, y)
            new_state = self.get_state('right', new_head, green_apples, positions)
            if self.check_valid(new_state):
        ...
        # Direction == right
            ...
            new_head = (x, y - 1)
            new_state = self.get_state('down', new_head, green_apples, positions)
            if self.check_valid(new_state):
                successors['SvrtiDesno'] = new_state

            new_head = (x, y + 1)
            new_state = self.get_state('up', new_head, green_apples, positions)
            if self.check_valid(new_state):
                successors['SvrtiLevo'] = new_state
...
if __name__ == '__main__':
    ...
    for i in range(num_green):
        green.append(tuple(map(int, input().split(","))))
    num_red = int(input())
    red = list()
    for i in range(num_red):
        red.append(tuple(map(int, input().split(","))))
    start_position = ((0, 7), (0, 8), (0, 9))
    initial = ('down', start_position, tuple(green))
    snake_problem = Snake(initial, tuple(red))
    result = breadth_first_graph_search(snake_problem)
