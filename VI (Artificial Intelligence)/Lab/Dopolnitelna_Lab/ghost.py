...
class GhostOnSkates(Problem):
    ...
    def successor(self, state):
        ...
        for i in range(1, 4):
            new_pos_up = (x, y + i)
            if self.check_valid(new_pos_up):
                successors['Gore ' + str(i)] = new_pos_up
            new_pos_right = (x + i, y)
            if self.check_valid(new_pos_right):
                successors['Desno ' + str(i)] = new_pos_right
    ...
    def h(self, node):
        x1, y1 = node.state
        x2, y2 = (self.n - 1, self.n - 1)
        man = self.manhattan_distance(x1, y1, x2, y2)
