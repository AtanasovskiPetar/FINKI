...
class Football(Problem):
    ...
    def goal_test(self, state):
        goal = [(7, 2), (7, 3)]
        return state[1] in goal

    @staticmethod
    def check_valid(state, opponents):
        man_pos = state[0]
        ball_pos = state[1]
        opponents_occupied_positions = set()
        for opponent in opponents:
            opponents_occupied_positions.add((opponent[0], int(opponent[1]) - 1))
            opponents_occupied_positions.add((opponent[0] + 1, opponent[1] - 1))
            opponents_occupied_positions.add((opponent[0] + 1, opponent[1]))
            opponents_occupied_positions.add((opponent[0] + 1, opponent[1] + 1))
            opponents_occupied_positions.add((opponent[0], opponent[1] + 1))
            opponents_occupied_positions.add((opponent[0] - 1, opponent[1] + 1))
            opponents_occupied_positions.add((opponent[0] - 1, opponent[1]))
            opponents_occupied_positions.add((opponent[0] - 1, opponent[1] - 1))

        rules = [0 <= man_pos[0] < 8, 0 <= man_pos[1] < 6, 0 <= ball_pos[0] < 8, 0 <= ball_pos[1] < 6,
                 man_pos != ball_pos, man_pos not in opponents, ball_pos not in opponents_occupied_positions]
        for rule in rules:
            if not rule:
                return False
        return True
    ...
    def successor(self, state):
        ...
        if ball_pos[0] == man_pos[0] and ball_pos[1] - 1 == man_pos[1]:
            new_ball_pos = (ball_pos[0], ball_pos[1] + 1)
            flag = True
        else:
        ...
        if ball_pos[0] == man_pos[0] and ball_pos[1] + 1 == man_pos[1]:
            ...
            new_ball_pos = ball_pos
        new_state = (new_man_pos, ball_pos)
        if self.check_valid(new_state, self.opponents):
            if flag:
        ...
        if ball_pos[1] == man_pos[1] and man_pos[0] + 1 == ball_pos[0]:
            new_ball_pos = (ball_pos[0] + 1, ball_pos[1])
            flag = True
        else:
            new_ball_pos = ball_pos
        new_state = (new_man_pos, new_ball_pos)
        ...
        if ball_pos[0] == man_pos[0] + 1 and ball_pos[1] == man_pos[1] + 1:
            new_ball_pos = (ball_pos[0] + 1, ball_pos[1] + 1)
            flag = True
        else:
            new_ball_pos = ball_pos
        new_state = (new_man_pos, new_ball_pos)
...
if __name__ == '__main__':
    ...
    opponents = [(3, 3), (5, 4)]
    start_state = tuple([man_pos, ball_pos])
    football_problem = Football(start_state, opponents)
    result = breadth_first_graph_search(football_problem)
