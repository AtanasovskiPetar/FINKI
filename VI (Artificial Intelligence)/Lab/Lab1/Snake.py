...
class Snake(Problem):
    ...
    @staticmethod
    def get_new_state(new_head_pos, apples, positions, direction):
    ...
    def check_valid(self, state):
        ...
        rule1 = 0 <= x < 10
        rule2 = 0 <= y < 10
        rule3 = head_position not in positions[1:]
        rule4 = head_position not in self.red_apples
        if 0 <= x < 10 and 0 <= y < 10 and head_position not in positions[1:] and head_position not in self.red_apples:
            return True
        else:
            return False
    ...
    def successor(self, state):
        ...
        if direction == 'dole':
            new_head_pos = (x, y - 1)
            new_state = self.get_new_state(new_head_pos, green_apples, positions, direction)
            if self.check_valid(new_state):
                successors['ProdolzhiPravo'] = new_state

            new_head_pos = (x - 1, y)
            new_state = self.get_new_state(new_head_pos, green_apples, positions, 'levo')
            if self.check_valid(new_state):
                successors['SvrtiDesno'] = new_state

            new_head_pos = (x + 1, y)
            new_state = self.get_new_state(new_head_pos, green_apples, positions, 'desno')
            if self.check_valid(new_state):
            ...
            new_head_pos = (x, y + 1)
            new_state = self.get_new_state(new_head_pos, green_apples, positions, 'gore')
            if self.check_valid(new_state):
                successors['SvrtiDesno'] = new_state

            new_head_pos = (x, y - 1)
            new_state = self.get_new_state(new_head_pos, green_apples, positions, 'dole')
            if self.check_valid(new_state):
                successors['SvrtiLevo'] = new_state
...
if __name__ == '__main__':
    ...
    initial = ('dole', ((0, 7), (0, 8), (0, 9)), tuple(green))

    problem = Snake(initial, tuple(red))
