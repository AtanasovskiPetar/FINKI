...
def valid(state):
    ...
    n_y = state[0][-1][0]
    n_x = state[0][-1][1]
    return n_y >= 0 and n_y < 10 and n_x >= 0 and n_x < 10 and ((n_y, n_x) not in state[2]) and len(state[0]) == len(
        set(state[0]))
...
class Snake(Problem):
    ...
    def successor(self, state):
        ...
        for d in directions:
            direction = get_direction(state[-1], d)
            new_pos = tuple([state[0][-1][0] + direction[0], state[0][-1][1] + direction[1]])
            if new_pos in state[1]:
                new_cords = tuple(list(state[0]) + [new_pos])
                new_green = list(state[1])
                new_green.remove(new_pos)
                new_green = tuple(new_green)
                successors[d] = tuple([new_cords, new_green, state[2], direction])
            else:
                new_cords = tuple(list(state[0])[1:] + [new_pos])
                new_state = tuple([new_cords, state[1], state[2], direction])
                if valid(new_state):
                    successors[d] = new_state
        return successors
...
if __name__ == '__main__':
    ...
    initial = tuple([tuple([(0, 0), (1, 0), (2, 0)]), tuple(green_apples), tuple(red_apples), (1, 0)])
    test = Snake(initial)
    ans1 = breadth_first_graph_search(test)
    print(ans1.solution())
