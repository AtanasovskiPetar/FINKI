...
if __name__ == '__main__':
    ...
    importance_list = list(decision_tree_classifier.feature_importances_)
    min_index = importance_list.index(min(importance_list))

    new_data = [row[:min_index] + row[min_index + 1:] for row in dataset]
    new_test_set = new_data[:int(percentage * len(new_data))]
    new_train_set = new_data[int(percentage * len(new_data)):]

    new_train_x = [row[:-1] for row in new_train_set]
    new_train_y = [row[-1] for row in new_train_set]
    new_test_x = [row[:-1] for row in new_test_set]
    new_test_y = [row[-1] for row in new_test_set]

    neural_network_1 = MLPClassifier(15, activation='relu', learning_rate_init=0.001, max_iter=200, random_state=0)
    neural_network_2 = MLPClassifier(15, activation='relu', learning_rate_init=0.001, max_iter=200, random_state=0)
    min_max_scaler = MinMaxScaler()
    standard_scaler = StandardScaler()
    min_max_scaler.fit(new_train_x)
    standard_scaler.fit(new_train_x)

    neural_network_1.fit(min_max_scaler.transform(new_train_x), new_train_y)
    neural_network_2.fit(standard_scaler.transform(new_train_x), new_train_y)

    prediction_mms = list(neural_network_1.predict(min_max_scaler.transform(new_test_x)))
    prediction_ss = list(neural_network_2.predict(standard_scaler.transform(new_test_x)))

    acc_mms, acc_ss = 0, 0
    for i in range(len(new_test_set)):
        if int(prediction_mms[i]) == new_test_y[i]:
            acc_mms += 1
        if int(prediction_ss[i]) == new_test_y[i]:
            acc_ss += 1

    acc_mms = acc_mms / len(new_test_set)
    acc_ss = acc_ss / len(new_test_set)
