...
if __name__ == "__main__":
    ...
    train_x = [row[:remove_index] + row[remove_index + 1:-1] for row in train_set]
    train_y = [row[-1] for row in train_set]

    test_set = dataset[int(0.85 * len(dataset)):]
    test_x = [row[:remove_index] + row[remove_index + 1:-1] for row in test_set]
    test_y = [row[-1] for row in test_set]

    n_trees = int(input())
    crit = input()
    classifier = RandomForestClassifier(criterion=crit, n_estimators=n_trees, random_state=0)
    ...
    entry = entry[:remove_index] + entry[remove_index+1:]
