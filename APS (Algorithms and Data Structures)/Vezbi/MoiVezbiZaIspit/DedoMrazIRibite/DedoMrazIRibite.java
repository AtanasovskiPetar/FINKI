package MoiVezbiZaIspit.DedoMrazIRibite;
/*
Дедо Мраз на распуст Problem 9 (0 / 0)
Во земјата Лапонија живее Дедо Мраз. Во слободното време кога не е Нова Година,
додека џуџињата си работат на играчките за следната година, Дедо Мраз има хоби.
Тој сака да одгледува рибички. Но тој своите рибички ги одгледува во природни езера.
...
Секоја пролет дедо мраз сака да прави порибување на езерцата со нови рибички.
Ваша задача е да му кажете на Дедо Мраз доколку тој пушти нови рибички во езерцето X,
во колку други езерца ќе можат рибичките сами да стигнат, а со тоа да нема потреба тој самиот да ги порибува тие езерца.

Влез: Во првата линија од влезот е даден број N < 15 бројот на езерца.
Во втората линија е даден број U < 20 бројот на реки меѓу езерцата.
Во следните U линии се дадени парови од 2 броја R и Q, што значи постои рекичка која тече од R до Q,
...
class Graph<E> {
    ...
    int sendFishes(int start) {
        ...
        return sendFishesRecursive(start, visited,0)-1;

    }

    private int sendFishesRecursive(int node, boolean[] visited, int counter) {
        if(visited[node]){
            return 0;
        }if(!visited[node]){
            counter=counter+1;
        ...
        for (int i = 0; i < num_nodes; i++) {
            if(adjacent(node, i)==1){
                if(!visited[i]){
                    counter = sendFishesRecursive(i,visited,counter);
                }
            }
        }
        return counter;
