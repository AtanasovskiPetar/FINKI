package MoiVezbiZaIspit.AVLSearchTree;

import java.io.BufferedReader;
import java.io.IOException;
import java.io.InputStreamReader;

public class AVLTreeExcercises {
    public static void main(String[] args) throws IOException {
        AVLTree<Integer> avlTree = new AVLTree<>();
        BufferedReader bf = new BufferedReader(new InputStreamReader(System.in));
        int n= Integer.parseInt(bf.readLine());
        for (int i = 0; i < n; i++) {
            avlTree.insert(Integer.parseInt(bf.readLine()));
        }

        avlTree.printTree();
        avlTree.printTreeWithLevels();
    }
}
...
class AVLTree<E extends Comparable<E>> {
    ...
    public void printTreeWithLevels(){
        printTreeWithLevels(root, 0);
    }

    private void printTreeWithLevels(AVLNode<E> t, int level){
        if(t==null){
            return;
        }
        ...
        System.out.println(t.info.toString());
        printTreeWithLevels(t.left, level+1);
        printTreeWithLevels(t.right, level+1);
    }
    /**
