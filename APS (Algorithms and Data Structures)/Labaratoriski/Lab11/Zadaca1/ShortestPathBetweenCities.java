package Lab11.Zadaca1;

// Zadacata e reshena bez dijksta.
// Napishan e algoritam za presmetka na najkratko rastojanie megju dve teminja (Dva grada)
...
class Graph<E> {
    ...
    public float findShortestPath(int startIndex, int endIndex){
        return findShortestPathRecursive(startIndex, endIndex, startIndex, 0);
    }
    public float weightOfAllNodes(){

        return (float) Arrays.stream(getAllEdges()).mapToDouble(edge->edge.getWeight()).sum();

    }
    public float findShortestPathRecursive(int startIndex, int endIndex,int curIndex, float curLength) {
        if(startIndex==endIndex){
            return curLength;
        }if(adjacent(curIndex, endIndex)==1){
            LinkedList<GraphNodeNeighbor<E>> neighbors = adjList[curIndex].getNeighbors();
            for (int i = 0; i < neighbors.size(); i++) {
                if(neighbors.get(i).node.getIndex()==endIndex){
                    return curLength+ neighbors.get(i).weight;
                }
            }
        }if(curLength>weightOfAllNodes()){
            return 10000000000.5f;
        }
        float minPath=10000000000.5f;
        float tmpPath;
        LinkedList<GraphNodeNeighbor<E>> neighbors = adjList[curIndex].getNeighbors();
        for (int i = 0; i < neighbors.size(); i++) {
            int index = neighbors.get(i).node.getIndex();
            if(index!=startIndex){
                tmpPath = findShortestPathRecursive(startIndex, endIndex,index,curLength+neighbors.get(i).weight);
                if(tmpPath<minPath){
                    minPath=tmpPath;
                }
            }
...
public class ShortestPathBetweenCities {
    public static void main(String[] args) throws IOException {
        ...
        int nunNodes = Integer.parseInt(bf.readLine());
        int numEdges = Integer.parseInt(bf.readLine());
        Graph<String> weightedGraph = new Graph<>(nunNodes);
        for (int i = 0; i < numEdges; i++) {
            ...
            weightedGraph.adjList[index1].setInfo(name1);
            weightedGraph.adjList[index2].setInfo(name2);
            weightedGraph.addEdge(index1, index2, weight);
        }
        String fromCity = bf.readLine();
        String toCity = bf.readLine();
        int startIndex=0;
        int endIndex=0;
        for (int i = 0; i < weightedGraph.num_nodes; i++) {
            if(weightedGraph.adjList[i].getInfo().equals(fromCity)){
                startIndex=i;
            }
            if(weightedGraph.adjList[i].getInfo().equals(toCity)){
                endIndex=i;
            }
        }

        float shortestPath = weightedGraph.findShortestPath(startIndex, endIndex) + weightedGraph.findShortestPath(endIndex, startIndex);
