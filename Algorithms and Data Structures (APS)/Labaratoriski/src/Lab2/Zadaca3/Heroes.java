...
public class Heroes {
    ...
    public static void startHeroesGame(SLL<Card> firstFriendCards, SLL<Card> secondFriendCards) {
        Card maxCard = firstFriendCards.getFirst().element;
        SLLNode<Card> curr = firstFriendCards.getFirst();
        while(curr!=null){
            if(curr.element.damage() > maxCard.damage()){
                maxCard=curr.element;
            }
            curr=curr.succ;
        }
        firstFriendCards.delete(firstFriendCards.find(maxCard));
        SLLNode<Card> secondListCurr = secondFriendCards.getFirst();
        for(int i=0;i<2;i++){
            secondListCurr=secondListCurr.succ;
        }
        secondFriendCards.insertAfter(maxCard,secondListCurr);
        //power x numAttacks
    }
