...
public class WorkplaceHierarchy {
    public static void main(String[] args) throws IOException {
        BufferedReader bf = new BufferedReader(new InputStreamReader(System.in));
        int n,m;
        n=Integer.parseInt(bf.readLine());
        m=Integer.parseInt(bf.readLine());
        Graph<String> workRelationsGraph = new Graph<>(n);
        for (int i = 0; i < m; i++) {
            ...
            workRelationsGraph.adjList[index1].setInfo(name1);
            workRelationsGraph.adjList[index2].setInfo(name2);
            float weight = Float.parseFloat(parts[4]);
            workRelationsGraph.addEdge(index1, index2, weight);
        }
        double minWeight = workRelationsGraph.prim(0).stream().mapToDouble(edge->edge.getWeight()).summaryStatistics().getSum();
        System.out.println((int) minWeight);
    }
}
