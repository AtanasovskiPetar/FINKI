...
class Timestamp <T> implements Comparable<Timestamp<?>>{
    ...
    public boolean equals(Object o) {
        ...
        return ((Timestamp<?>) o).getTime().compareTo(getTime())==0;
    }

}
class Scheduler<T>{
    List<Timestamp<T>> times;
    T type;
    ...
    public Timestamp<T> next(){
        Timestamp timeNow = new Timestamp<>(LocalDateTime.now(), "test");
        return times.stream().filter(p->p.compareTo(timeNow)>0).min(Comparator.naturalOrder()).orElse(times.get(0));
    }
    public Timestamp<T> last(){
        Timestamp timeNow = new Timestamp<>(LocalDateTime.now(), "test");
        return times.stream().filter(p->p.compareTo(timeNow)<0).max(Comparator.naturalOrder()).orElse(times.get(0));
    }
    public List<Timestamp<T>> getAll(LocalDateTime begin, LocalDateTime end){
        Timestamp timeBegin = new Timestamp<>(begin, "test");
        Timestamp timeEnd = new Timestamp<>(end, "test");
        return times.stream().filter(p->p.compareTo(timeBegin)>0).filter(a->a.compareTo(timeEnd)<0).collect(Collectors.toList());
    }
}
