...
class LogSystem <T>{
    ...
    void printResults() {
        ...
        LogProcessor<ILog> firstLogProcessor = p -> {
            List<ILog> newLogs = p.stream().filter(a -> a.getType().equals("INFO")).collect(Collectors.toList());
            return new ArrayList<>(newLogs);
        };

        LogProcessor<ILog> secondLogProcessor = p -> {
            List<ILog> newLogs = p.stream().filter(a -> a.getMessage().length()<100).collect(Collectors.toList());
            return new ArrayList<>(newLogs);
        };

        LogProcessor<ILog> thirdLogProcessor = p -> {
            List<ILog> newLogs = p.stream().sorted(Comparator.comparing(ILog::getTimestamp)).collect(Collectors.toList());
            return new ArrayList<>(newLogs);
        };

        System.out.println("RESULTS FROM THE FIRST LOG PROCESSOR");
        firstLogProcessor.processLogs((ArrayList<ILog>) logsList).forEach(l -> System.out.println(l.toString()));

        System.out.println("RESULTS FROM THE SECOND LOG PROCESSOR");
        secondLogProcessor.processLogs((ArrayList<ILog>) logsList).forEach(l -> System.out.println(l.toString()));

        System.out.println("RESULTS FROM THE THIRD LOG PROCESSOR");
        thirdLogProcessor.processLogs((ArrayList<ILog>) logsList).forEach(l -> System.out.println(l.toString()));
    }
}
