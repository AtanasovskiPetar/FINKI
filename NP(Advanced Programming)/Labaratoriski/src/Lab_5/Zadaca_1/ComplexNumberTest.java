...
class ComplexNumber <R extends Number, I extends Number> implements Comparable<ComplexNumber>{
    ...
    public String toString() {
        double r = real.doubleValue();
        double i = imaginary.doubleValue();
        //return String.format("%.2f+%2.fi",r,i); //0.00+0.00i
        StringBuilder sb = new StringBuilder();
        sb.append(String.format("%.2f",r));
        if(i >= 0){
            sb.append("+");
        }
        sb.append(String.format("%.2f",i)).append("i");
        return sb.toString();
    }
    ...
    public int compareTo(ComplexNumber o) {
        if(modul() > o.modul()) return 1;
        else if (modul() == o.modul()) return 0;
        else return -1;
    }
}
